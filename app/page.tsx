...
export default function HomePage() {
  ...
  const buscarDetalhesProduto = async (produtoId: string, unidadeId: string): Promise<Produto | null> => {
    try {
      // Busca o produto específico na unidade específica
      // Nota: Ajuste a URL conforme sua API real. Estou assumindo uma busca filtrada.
      const response = await fetch(`/api/produtos/buscar?busca=${produtoId}&mercado=${unidadeId}&limit=1`, {
        cache: 'no-store'
      });
      ...
      const data = await response.json();
      // A API de busca retorna um array, pegamos o primeiro que corresponder
      // Precisamos filtrar client-side se a API não filtrar exato por ID
      const produtosEncontrados = Array.isArray(data) ? data : [];
      const produto = produtosEncontrados.find((p: any) =>
        (p.produto?.id === produtoId || p.id === produtoId) &&
        (p.unidade?.id === unidadeId)
      );

      if (!produto) return null;

      // Normaliza o produto (reutilizando a lógica do buscarProdutos se possível, ou simplificando)
      return {
        ...produto,
        nome: produto.nome || produto.produto?.nome,
  ...
  const handleAceitarRecomendacao = async (produtoId: string, unidadeId: string) => {
    try {
      // 1. Identificar se o produto já está na lista (pelo produtoId genérico)
      // A recomendação geralmente sugere trocar um produto da lista por outro melhor
      // OU sugere um produto novo.
      // Assumindo que ModuloIA sugere uma MELHOR OFERTA para um produto que o usuário já tem interesse
      // ou que é similar a algo na lista.

      // Vamos buscar os detalhes do produto recomendado
      const novoProduto = await buscarDetalhesProduto(produtoId, unidadeId);

      if (!novoProduto) {
        error('Não foi possível encontrar os detalhes da oferta selecionada.');
        return;
      }

      // 2. Verificar se já existe um produto similar na lista para substituir
      // A lógica de "substituição" depende de sabermos QUAL item da lista está sendo substituído.
      // Se a IA não manda o ID do item original, assumimos que é uma adição ou substituição por "mesmo produto, outra unidade"

      const itemExistenteIndex = listaProdutos.findIndex(p => p.produto.id === produtoId);

      if (itemExistenteIndex >= 0) {
        // Substituir
        const novaLista = [...listaProdutos];
        novaLista[itemExistenteIndex] = novoProduto;
        setListaProdutos(novaLista);
        success(`Oferta aceita! Produto atualizado para ${novoProduto.unidade.mercado.nome}`);
      } else {
        // Adicionar novo
        adicionarALista(novoProduto);
        success(`Oferta adicionada à sua lista!`);
      }
      ...
      error('Ocorreu um erro ao processar a recomendação.');
    }
  };
