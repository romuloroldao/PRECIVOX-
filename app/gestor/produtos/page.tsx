...
export default function GestorProdutosPage() {
  ...
  }, [debouncedSearch, categoriaFilter, unidadeFilter, page, session]);
  ...
  const loadUnidades = async () => {
    try {
      // Carregar apenas unidades do mercado do gestor
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          const unidadesPromises = mercados.map((mercado: any) =>
            fetch(`/api/markets/${mercado.id}/unidades`)
              .then((res) => res.json())
              .then((data) => (data.success ? data.data || [] : []))
              .catch(() => [])
          );

          const unidadesArrays = await Promise.all(unidadesPromises);
          const todasUnidades = unidadesArrays.flat();
                      ...
                      <tr key={produto.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex space-x-2">
                            <Link
                              href={`/gestor/produtos/${produto.id}`}
