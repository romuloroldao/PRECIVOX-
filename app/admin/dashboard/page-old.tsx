...
import { ErrorDisplay, useErrorHandler } from '@/components/ErrorBoundary';
...
export default function AdminDashboardPage() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      setStatsError(null);
      const result = await apiFetch<UserStats>('/api/admin/stats', {
        retries: 2,
        retryDelay: 1000,
        timeout: 10000,
      });
      ...
      if (result.success && result.data) {
        ...
        setStatsError(result.error?.message || 'Erro ao carregar estatísticas');
      }
    } catch (error) {
      console.error('Erro ao buscar estatísticas:', error);
      setStatsError('Erro inesperado ao carregar estatísticas');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const fetchRecentUsers = useCallback(async () => {
    try {
      setUsersError(null);
      const result = await apiFetch<RecentUser[]>('/api/admin/recent-users', {
        retries: 2,
        retryDelay: 1000,
        timeout: 10000,
      });
      ...
      if (result.success && result.data) {
        ...
        setUsersError(result.error?.message || 'Erro ao carregar usuários recentes');
      }
    } catch (error) {
      console.error('Erro ao buscar usuários recentes:', error);
      setUsersError('Erro inesperado ao carregar usuários recentes');
    }
  }, []);
