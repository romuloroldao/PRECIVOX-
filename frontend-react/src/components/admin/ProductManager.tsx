// src/components/admin/ProductManager.tsx - Gestão de Produtos Admin
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setProducts([]); // Banco vazio para gestão manual
      }
    } catch (error) {
      console.error('Erro na requisição:', error);
      setProducts([]); // Banco vazio para gestão manual
      showToast('Sistema configurado para gestão manual de produtos', 'info');
    } finally {
      setLoading(false);
    }
  };
      ...
      if (response.ok) {
        ...
        showToast(
          <>
            ...
            Imagem regenerada via IA!
          </>,
          'success'
        );
        loadProducts(); // Recarregar para mostrar nova imagem
      } else {
        throw new Error('Erro ao regenerar imagem');
      }
    } catch (error) {
      console.error('Erro:', error);
      showToast('Erro ao regenerar imagem via IA', 'error');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {products.filter(p => p.promocao).length}
                </p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {new Set(products.map(p => p.mercado)).size}
                </p>
              </div>
            </div>
          </div>
