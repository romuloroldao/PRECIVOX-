...
function getJwtSecret() {
  ...
  if (process.env.NODE_ENV === 'test') {
    const testSecret = process.env.JWT_SECRET?.trim() || 'test-secret-key-for-jest-min-16';
  ...
  if (process.env.NODE_ENV === 'development') {
    throw new Error(
      'JWT_SECRET (mín. 16 caracteres) é obrigatório em desenvolvimento. ' +
        'Copie .env.example para .env.local e defina um valor seguro.',
    );
  }

  throw new Error(
    'JWT_SECRET deve estar configurado em produção (mín. 16 caracteres, sem placeholders).',
  );
}
